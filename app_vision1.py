...
def ensure_panel_folder():
    ...
    Creates it once on first call; returns the same path on every
    subsequent call. Never creates a second folder.

    ✅ FIX: removed seq1_snapshot_data guard — snapshot is never
    committed because best.pt has no serial_number class. The folder
    is now created as soon as current_sequence >= 1 (SEQ1 active).
    panel_id is set once (in allocate_panel_folder_for_seq1) and
    reused, so only one timestamp folder is ever created per panel.
    """
    # If folder already created for this panel — return it immediately
    if state.current_sequence_panel_folder is not None:
        return state.current_sequence_panel_folder

    # Set panel_id once (timestamp) if not already set
    if not state.panel_id:
        if state.current_sequence < 1:
            # No active sequence yet — too early, don't create orphan folder
            return None
        state.panel_id = datetime.now().strftime("%H%M%S")

    # Create folder exactly once
    state.current_sequence_panel_folder = get_panel_folder(
        state.serial_number, state.panel_id)
    if state.current_sequence_panel_folder:
        print(f"[FOLDER] ✅ Panel folder created: {state.current_sequence_panel_folder}",
              flush=True)
        if _CAM2_OCR_AVAILABLE and camera2_ocr_instance is not None:
            camera2_ocr_instance.set_panel_folder(
                state.current_sequence_panel_folder)

    return state.current_sequence_panel_folder

def allocate_panel_folder_for_seq1(serial_present=False):
    """Create the panel save folder as soon as SEQ1 is stable.
    Reuses existing folder/panel_id — never creates a second folder.
    """
    # Already created — return immediately (idempotent)
    if state.current_sequence_panel_folder is not None:
        return state.current_sequence_panel_folder

    if (state.seq1_detection_count >= 2
            and state.current_sequence in (0, 1)):
        # Reuse panel_id if already set (ensure_panel_folder may have set it)
        if not state.panel_id:
            state.panel_id = datetime.now().strftime("%H%M%S")
        state.current_sequence_panel_folder = get_panel_folder(
        ...
        print(f"[FOLDER] ✅ Panel folder created: {state.current_sequence_panel_folder}",
              flush=True)
        print(f"[FOLDER]    panel_id={state.panel_id}", flush=True)
        if _CAM2_OCR_AVAILABLE and camera2_ocr_instance is not None:
            camera2_ocr_instance.set_panel_folder(state.current_sequence_panel_folder)
            print(f"[FOLDER]    Camera-2 folder set", flush=True)
    ...
    if wiping_confirmed:
        ...
        if cur == 1:
            ...
            if state.seq1_wiping_frames >= 3 and not state.ocr_started:
                # ✅ FIX: removed seq1_snapshot_data gate — snapshot never
                # committed (best.pt has no serial_number class). Trigger
                # OCR at frame 3 unconditionally; camera2 serial.pt handles
                ...
                if folder and camera2_ocr_instance is not None:
                    ...
                    print(f'[CAM2-OCR] ✅ OCR triggered at SEQ1 wiping frame '
                          f'{state.seq1_wiping_frames}', flush=True)
...
def process_frame(frame, detections):
    ...
    if (_panel_is_portrait
            ...
            and _CAM2_OCR_AVAILABLE
            and camera2_ocr_instance is not None):
        # ✅ FIX: removed seq1_snapshot_data guard — never set because best.pt
        # has no serial_number class. Portrait + SEQ1 is sufficient trigger.
        folder = ensure_panel_folder()
        if folder:
            camera2_ocr_instance.set_panel_folder(folder)
            camera2_ocr_instance.start_ocr()
            ...
            print(f'[CAM2-OCR]    folder: {folder}', flush=True)
    elif _panel_is_portrait and state.current_sequence == 1:
        pass  # ocr_started already True or cam2 not available
