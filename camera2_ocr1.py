...
class Camera2OCR:
    ...
    def _ocr_pipeline(self, crop) -> str:
        ...
        # ── resolve save dir (only inside panel_folder — no script-dir fallback) ──
        # ✅ FIX: removed fallback to os.path.dirname(__file__)/serial_captures.
        # That fallback caused captures to land next to camera2_ocr.py instead of
        # inside the timestamped panel folder, creating a second folder.
        # If panel_folder is not set yet, sd=None and saves are skipped until it is.
        if self.panel_folder and self.panel_folder not in ('.', ''):
            ...
            sd = None   # panel_folder not set yet; skip saves this frame
